...
class MessageSerialiser
{
     ...
     */
    func getMessages(withIdentifiers: [String], completionHandler: @escaping(_ returnedMessages: [Message]?, _ errorDescriptors: [String]?) -> Void)
    {
            ...
            for individualIdentifier in withIdentifiers
            {
                getMessage(withIdentifier: individualIdentifier) { (wrappedMessage, wrappedErrorDescriptor) in
                    ...
                    if messageArray.count + errorDescriptorArray.count == withIdentifiers.count
                    {
                        ...
                        completionHandler(messageArray.count == 0 ? nil : messageArray, errorDescriptorArray.count == 0 ? nil : errorDescriptorArray)
                    }
                }
            }
        }
