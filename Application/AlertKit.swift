...
class AlertKit
{
                              ...
                              networkDependent: Bool)
    {
        DispatchQueue.main.async {
            ...
            {
                guard validateMetaData(metaData) else { printReport("Improperly formatted metadata.", errorCode: nil, isFatal: false, metaData: [#file, #function, #line]); return }
                        ...
                        DispatchQueue.main.async {
                            ...
                            for individualKey in sortedKeyArray
                            {
                                ...
                                if iterationCount == 0
                                {
                                    ...
                                    {
                                        guard errorAlertController.actions.count > preferredSelector + 1 else { printReport("Preferred Selector index was out of range of the provided Selectors.", errorCode: nil, isFatal: false, metaData: [#file, #function, #line]); return }
