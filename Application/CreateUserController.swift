...
class CUC: UIViewController
{
    ...
    override func viewDidLoad()
    {
        ...
        for individualTextField in textFieldArray
        {
            ...
            individualTextField.tintColor = getColour(withHexadecimalString: "#0097EF")
        }
    ...
    override func viewWillAppear(_ animated: Bool)
    {
        ...
        doneButton.initialiseLayer(animateTouches: true, backgroundColour: getColour(withHexadecimalString: "#60C129"), customBorderFrame: nil, customCornerRadius: nil, shadowColour: getColour(withHexadecimalString: "#3B9A1B").cgColor)
    ...
    @objc func keyboardDidShow(_ withNotification: Notification)
    {
        ...
        if let keyboardFrame: NSValue = withNotification.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue
        {
            ...
            //Unwrap the «CreateAccountController's» parameters.
            guard let createAccountController = self.parent as? CreateAccountController else { printReport("No CreateAccountController.", errorCode: nil, isFatal: true, metaData: [#file, #function, #line]); return }
            
            guard let instructionTextView = createAccountController.instructionTextView else { printReport("No instructionTextView.", errorCode: nil, isFatal: true, metaData: [#file, #function, #line]); return }
