...
class CreateAccountController: UIViewController, MFMailComposeViewControllerDelegate, SSRadioButtonControllerDelegate
{
    ...
    override func viewDidLoad()
    {
        ...
        view.addShadowBorder(backgroundColour: .white, borderColour: getColour(withHexadecimalString: "#E1E0E1").cgColor, withFrame: CGRect(x: adoptButton.frame.origin.x - 5, y: adoptButton.frame.origin.y, width: adoptButton.frame.size.width, height: adoptButton.frame.size.height), withTag: aTagFor("adoptButton_BORDER"))
        view.addShadowBorder(backgroundColour: .white, borderColour: getColour(withHexadecimalString: "#E1E0E1").cgColor, withFrame: CGRect(x: shelterButton.frame.origin.x - 5, y: shelterButton.frame.origin.y, width: shelterButton.frame.size.width, height: shelterButton.frame.size.height), withTag: aTagFor("shelterButton_BORDER"))
    ...
    override func viewWillAppear(_ animated: Bool)
    {
        if lastInitialisedController != self
        {
            ...
            backButton.initialiseLayer(animateTouches: true, backgroundColour: getColour(withHexadecimalString: "#E95A53"), customBorderFrame: nil, customCornerRadius: nil, shadowColour: getColour(withHexadecimalString: "#D5443B").cgColor)
            backButton.initialiseTranslation(allowedToAdjust: true, alternateString: nil, backUp: nil, useActivityIndicator: true)
            
            continueButton.isEnabled = false
            continueButton.initialiseLayer(animateTouches: true, backgroundColour: getColour(withHexadecimalString: "#60C129"), customBorderFrame: nil, customCornerRadius: nil, shadowColour: getColour(withHexadecimalString: "#3B9A1B").cgColor)
    ...
    func presentBirthdateController()
    {
        ...
        guard let adoptButtonShadow   = self.view.findSubview(aTagFor("adoptButton_BORDER")) else { printReport("No adopt button border.", errorCode: nil, isFatal: true, metaData: [#file, #function, #line]); return }
        guard let shelterButtonShadow = self.view.findSubview(aTagFor("shelterButton_BORDER")) else { printReport("No shelter button border.", errorCode: nil, isFatal: true, metaData: [#file, #function, #line]); return }
    ...
    func createAccount()
    {
        Auth.auth().createUser(withEmail: self.createUserController.eMailTextField.text!, password: self.createUserController.passwordTextField.text!, completion: { (wrappedReturnedUser, wrappedReturnedError) in
            if let returnedError = wrappedReturnedError
            {
                if returnedError.localizedDescription == "The email address is already in use by another account."
                {
                    PresentationManager().errorAlertController(withTitle: "Already Registered",
                                                               withMessage: "The e-mail address is already in use with another account.",
                ...
                else
                {
                    PresentationManager().errorAlertController(withTitle: nil,
                                                               withMessage: nil,
                                                               extraneousInformation: errorInformation(forError: returnedError as NSError),
                                                               withCancelButtonTitle: nil,
                                                               withAlternateSelectors: nil,
                                                               preferredActionIndex: nil,
                                                               withFileName: #file,
            ...
            else
            {
                if let returnedUser = wrappedReturnedUser
                {
                    ...
                    let livingSpaceImageViews = [self.biographicController.firstLivingSpaceImageView, self.biographicController.secondLivingSpaceImageView, self.biographicController.thirdLivingSpaceImageView]
                    
                    for imageView in livingSpaceImageViews
                    {
                        if let imageData = imageView?.image?.jpegData(compressionQuality: 0.5)
                        {
                            if livingSpaceImageData == nil
                            {
                                livingSpaceImageData = [imageData.base64EncodedString()]
                            }
                            else
                            {
                                livingSpaceImageData!.append(imageData.base64EncodedString())
                            }
                        }
                    }
                    
                    let profileImageData = self.biographicController.profileImageView.image?.jpegData(compressionQuality: 0.5)?.base64EncodedString()
                    
                    let associatedInterview = self.processInterview()
                    
                    UserSerialiser().createUser(associatedBiography:  (self.biographicController.biographyTextView.text! == (translationArchive["Add a short statement about yourself to be shown to shelters"] ?? "Add a short statement about yourself to be shown to shelters") ? "!" : self.biographicController.biographyTextView.text!),
                                                associatedIdentifier: returnedUser.user.uid,
                                                associatedInterview:  associatedInterview,
                                                birthDate:            self.birthdateController.birthdatePicker.date,
                                                emailAddress:         self.createUserController.eMailTextField.text!,
                                                firstName:            self.createUserController.firstNameTextField.text!,
                                                lastName:             self.createUserController.lastNameTextField.text!,
                                                livingSpaceImageData: livingSpaceImageData,
                                                phoneNumber:          self.createUserController.phoneNumberTextField.text!.digits,
                                                profileImageData:     profileImageData,
                                                zipCode:              Int(self.createUserController.postalCodeTextField.text!)!)
                    { (wrappedReturnedUser, createUserError) in
                        if wrappedReturnedUser != nil
                        {
                            let activityIndicator = self.view.findSubview(aTagFor("activityIndicator")) as! UIActivityIndicatorView
                            
                            activityIndicator.removeFromSuperview()
                            
                            PKHUD.sharedHUD.contentView = PKHUDSuccessView(title: nil, subtitle: nil)
                            PKHUD.sharedHUD.show(onView: self.view)
                            
                            DispatchQueue.main.asyncAfter(deadline: .now() + .milliseconds(1500)) {
                                hideHud()
                                self.performSegue(withIdentifier: "signInFromCreateAccountSegue", sender: self)
                            }
                        }
                        else
                        {
                            printReport(createUserError ?? "An unknown error occurred.", errorCode: nil, isFatal: false, metaData: [#file, #function, #line])
                            
                            PresentationManager().errorAlertController(withTitle: nil,
                                                                       withMessage: nil,
                                                                       extraneousInformation: createUserError,
                                                                       withCancelButtonTitle: nil,
                                                                       withAlternateSelectors: nil,
                                                                       preferredActionIndex: nil,
                                                                       withFileName: #file,
                ...
                {
                    printReport("No error, but no returned user either.", errorCode: nil, isFatal: false, metaData: [#file, #function, #line])
                    
                    PresentationManager().errorAlertController(withTitle: nil,
                                                               withMessage: nil,
                                                               extraneousInformation: "No eror, but no returned user either.",
                                                               withCancelButtonTitle: nil,
                                                               withAlternateSelectors: nil,
                                                               preferredActionIndex: nil,
                                                               withFileName: #file,
    ...
    func processInterview() -> Interview
    {
        let confidenceLevel = Int(confidenceController.confidenceSlider.value)
        let employmentStatus = (employmentController.employmentStatusSegmentedControl.selectedSegmentIndex != UISegmentedControl.noSegment) ? employmentController.employmentStatusSegmentedControl.selectedSegmentIndex : 3
        let employmentStatusExplanation = (employmentController.somethingElseTextView.text! == "" ? "!" : employmentController.somethingElseTextView.text!)
        
        var traitsArray: [String]! = []
        
        for individualSubview in traitsController.view.subviews
        {
            if individualSubview.tag == aTagFor("traitsEncapsulatingView")
            {
                for shadowButton in individualSubview.subviews
                {
                    if let individualButton = shadowButton as? ShadowButton
                    {
                        if individualButton.backgroundColor == getColour(withHexadecimalString: "#60C129")
                        {
                            traitsArray.append(individualButton.titleLabel!.text!)
                        }
                        else if individualButton.backgroundColor == getColour(withHexadecimalString: "#E95A53")
                        {
                            traitsArray.append("NOT \(individualButton.titleLabel!.text!)")
                        }
                    }
                }
            }
        ...
        let livingSituation = (livingSituationController.livingSituationSegmentedControl.selectedSegmentIndex != UISegmentedControl.noSegment) ? livingSituationController.livingSituationSegmentedControl.selectedSegmentIndex : 3
        let livingSituationExplanation = (livingSituationController.somethingElseTextView.text! == "" ? "!" : livingSituationController.somethingElseTextView.text!)
        var otherPets = confidenceController.otherPetsTextView.text!.components(separatedBy: ",")
        
        for individualPet in otherPets
        {
            if individualPet.hasPrefix(" ")
            {
                #warning("Is firstIndex(of: ) okay here?")
                otherPets[otherPets.firstIndex(of: individualPet)!] = individualPet.stringCharacters[1...(individualPet.stringCharacters.count - 1)].joined()
            }
        }
        
        let finalInterview = Interview(confidenceLevel:             confidenceLevel,
                                       employmentStatus:            employmentStatus,
                                       employmentStatusExplanation: (employmentStatusExplanation == "!" ? nil : employmentStatusExplanation),
                                       lifestyleTraits:             traitsArray,
                                       livingSituation:             livingSituation,
                                       livingSituationExplanation:  (livingSituationExplanation == "!" ? nil : livingSituationExplanation),
                                       otherPets:                   (confidenceController.otherPetsTextView.text! == (translationArchive["Please separate your entries with a comma"] ?? "Please separate your entries with a comma") ? ["!"] : otherPets))
    ...
    func setTraitsInstructionText(firstPart: String, tapOnce: String, secondPart: String, tapTwice: String, thirdPart: String)
    {
        ...
        let tapOnceAttributes: [NSAttributedString.Key:Any] = [
            .font: UIFont(name: "SFUIText-Regular", size: 13)!,
            .foregroundColor: getColour(withHexadecimalString: "#60C129"),
            .paragraphStyle: paragraphStyle]
        
        let tapTwiceAttributes: [NSAttributedString.Key:Any] = [
            .font: UIFont(name: "SFUIText-Regular", size: 13)!,
            .foregroundColor: getColour(withHexadecimalString: "#E95A53"),
            .paragraphStyle: paragraphStyle]
...
extension UIView
{
     ...
     */
    func removeSubview(_ withTag: Int)
    {
